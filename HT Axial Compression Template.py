...
#Plot for temperature evolution with test
df_t = (df_failure2.loc[:,
                          (df_failure2.columns.str.contains('TC01')) | (df_failure2.columns.str.contains('TC02')) | (df_failure2.columns.str.contains('TC03')) |
                          (df_failure2.columns.str.contains('TC04')) | (df_failure2.columns.str.contains('TC05')) | (df_failure2.columns.str.contains('TC06'))])
plt.figure(1, figsize=(10.17, 6.64), dpi=160)
df_t.insert(0, 'Time', df_failure2['Scan #']/(10*60))
df_t.plot(x='Time')
plt.subplots_adjust(left=0.1, right=0.95, bottom=0.2, top=0.95)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.ylim([100,133])
plt.xlabel('Time (min)', fontsize=14)  # X axis labeling and font size
plt.ylabel('Temperature $^\circ$C', fontsize=14)  # Y axis labeling and font size
plt.title('Temperature profile during test', fontsize=14)  # Chart title
plt.legend(fontsize = 12, labels = ['TC01 45$^\circ$ axis Centre OD', 'TC02 225$^\circ$ axis Centre OD', 'TC03 270$^\circ$ axis Centre ID', 'TC04 90$^\circ$axis Centre ID', 'TC05 45$^\circ$axis End A OD', 'TC06 45$^\circ$ axis End B OD'], loc='lower center', bbox_to_anchor=(0.5, -0.25), ncol=3)
plt.grid(color = 'lightgrey', ls = '-.', linewidth = 0.25)
plt.savefig(dir/'Failure Temperature Profile.svg', format = "svg")
pdf.savefig()
plt.show()
...
fig.subplots_adjust(left=0.05, right=0.9, bottom=0.15, top=0.95)
# Save the plot and show
plt.savefig(dir / 'Stress vs String Pot Strain.svg', format='svg')
pdf.savefig()
plt.show()
...
plt.yticks(fontsize=12)
plt.xlabel('Time (min)', fontsize=14)  # X axis labeling and font size
plt.ylabel('Strain (%)', fontsize=14)  # Y axis labeling and font size
...
ax.yaxis.set_label_coords(0.9, 0.82)
# Adjust the plot layout
...
ax.axvline(x=-30.0, color='black', linewidth=1.25)
# Save the plot and show
plt.grid(color='lightgrey', ls='-.', linewidth=0.25)
...
ax1.plot(dffailure, df_failure['Stress'])
ax1.set_xlabel('Principal Angle ($^\circ$)', fontsize=14)  # X axis labeling and font size
ax1.set_ylabel('Stress (MPa)', fontsize=14)  # Y axis labeling and font size
ax1.tick_params(axis='y')
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
...
#Plot for temperature evolution with test
file_modulus_gauges_t = (file_modulus.loc[:,
                          (file_modulus.columns.str.contains('TC01')) | (file_modulus.columns.str.contains('TC02')) | (file_modulus.columns.str.contains('TC03')) |
                          (file_modulus.columns.str.contains('TC04')) | (file_modulus.columns.str.contains('TC05')) | (file_modulus.columns.str.contains('TC06'))])
plt.figure(1, figsize=(10.17, 6.64), dpi=160)
file_modulus_gauges_t.insert(0, 'Time', file_modulus['Scan #']/(10*60))
file_modulus_gauges_t.plot(x='Time')
plt.subplots_adjust(left=0.1, right=0.95, bottom=0.2, top=0.95)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.ylim([100,133])
...
plt.legend(fontsize = 12, labels = ['TC01 45$^\circ$ axis Centre OD', 'TC02 225$^\circ$ axis Centre OD', 'TC03 270$^\circ$ axis Centre ID', 'TC04 90$^\circ$axis Centre ID', 'TC05 45$^\circ$axis End A OD', 'TC06 45$^\circ$ axis End B OD'], loc='lower center', bbox_to_anchor=(0.5, -0.25), ncol=3)
plt.grid(color = 'lightgrey', ls = '-.', linewidth = 0.25)
plt.savefig(dir/'Modulus Temperature Profile.svg', format = "svg")
pdf.savefig()
plt.show()
...
cycle4 = df1.loc[valleys_list[2]:peaks_list[3]]
...
diff = diff1/x_new
index = np.where(diff[2500:] >= 0.2)[0][0]+2500 #Find the index value where the difference first reaches 0.2%, but skips the first 700 rows
